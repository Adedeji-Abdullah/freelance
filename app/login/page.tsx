...
const Login = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await resp.json().catch(() => (console.log("error logging in")));
      console.log(data)
  ...
  return (
    <div className="flex flex-col flex-1 items-center justify-center bg-blend-color-dodge font-sans">
      <div className="px-28 py-20 rounded-2xl shadow-xl md:shadow-2xl shadow-white border opacity-40 absolute">
        <h1 className="text-3xl mb-10 font-bold text-center">SIGN IN</h1>
      <form action="submit">
        <div className="mb-4">
          <label className="text-xl block opacity-100" htmlFor="email">Email</label>
          <input 
            className="block border-2 rounded-md h-12 w-full mt-2" 
            type="email" 
            id="email"
            value={email}
